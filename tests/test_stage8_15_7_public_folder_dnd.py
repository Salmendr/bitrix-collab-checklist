...
class PublicFolderStaticContractTests(unittest.TestCase):
    ...
    def test_share_button_is_between_upload_and_folder_but_disabled(self):
        ...
        share_block = source[share_at:source.index("</button>", share_at)]
        self.assertIn("Поделиться ссылкой — временно недоступно", share_block)
        self.assertIn("aria-disabled=\"true\"", share_block)
        self.assertIn("disabled", share_block)
        self.assertNotIn("checklist-action-button-share-label", share_block)
    ...
    def test_all_three_upload_surfaces_have_drag_and_drop(self):
        ...
        self.assertIn("ChecklistUploadStaging", popup)
        self.assertIn("ChecklistUploadStaging.create", folder)
        for source in (staging, public):
